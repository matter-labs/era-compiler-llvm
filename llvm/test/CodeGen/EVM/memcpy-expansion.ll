...
define fastcc void @expand-known(ptr %dest, ptr %src) {
; Loop
; CHECK:   %loop-index = phi i256 [ 0, %{{.*}} ], [ [[NEWCTR:%[0-9]+]], %load-store-loop ]
; CHECK:   [[REG:%[0-9]+]] = load i256, ptr %{{[0-9]+}}, align 1
; CHECK:   store i256 [[REG]], ptr %{{[0-9]+}}, align 1
; CHECK:   [[NEWCTR]] = add i256 %loop-index, 1

; Resudual
; CHECK:   [[PART1:%[0-9]+]] = load i256, ptr %{{[0-9]+}}, align 1
; CHECK:   [[PART1_M:%[0-9]+]] = and i256 [[PART1]], -{{[0-9]+}}
; CHECK:   [[PART2:%[0-9]+]] = load i256, ptr %{{[0-9]+}}, align 1
; CHECK:   [[PART2_M:%[0-9]+]] = and i256 [[PART2]], {{[0-9]+}}
; CHECK:   [[RES:%[0-9]+]] = or i256 [[PART1_M]], [[PART2_M]]
; CHECK:   store i256 [[RES]], ptr %{{[0-9]+}}, align 1
  call void @llvm.memcpy.p0i256.p0i256.i256(ptr %dest, ptr %src, i256 42, i1 false)
  ret void
}

; CHECK-LABEL: expand-unknown
define fastcc void @expand-unknown(ptr %dest, ptr %src, i256 %size) {
...
; Residual
; CHECK:   %{{[0-9]+}} = load i256, ptr %{{[0-9]+}}, align 1
; CHECK:   %{{[0-9]+}} = load i256, ptr %{{[0-9]+}}, align 1
...
; CHECK:   br i1 [[COND2]], label %{{.*}}, label %{{.*}}
  call void @llvm.memcpy.p0i256.p0i256.i256(ptr %dest, ptr %src, i256 %size, i1 false)
  ret void
}
