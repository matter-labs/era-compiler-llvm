...
define i256 @sha3(ptr %offset, i256 %size) nounwind {
; CHECK-LABEL: @sha3
; CHECK: ARGUMENT [[IN2:\$[0-9]+]], 1
; CHECK: ARGUMENT [[IN1:\$[0-9]+]], 0
; CHECK: SHA3 [[RES1:\$[0-9]+]], [[IN1]], [[IN2]]

  %res = call i256 @llvm.evm.sha3(ptr %offset, i256 %size)
  ret i256 %res
}
...
define i256 @calldataload(i256 %rs1) nounwind {
; CHECK-LABEL: @calldataload
; CHECK: ARGUMENT [[IN1:\$[0-9]+]], 0
; CHECK: CALLDATALOAD [[RES1:\$[0-9]+]], [[IN1]]

  %res = call i256 @llvm.evm.calldataload(i256 %rs1)
  ret i256 %res
}

define void @calldatacopy(ptr %dst, i256 %src, i256 %size) nounwind {
...
; CHECK: CALLDATACOPY [[IN1]], [[IN2]], [[IN3]]
  ...
  call void @llvm.evm.calldatacopy(ptr %dst, i256 %src, i256 %size)
  ret void
}
...
define void @codecopy(ptr %dst, i256 %src, i256 %size) nounwind {
...
; CHECK: CODECOPY [[IN1]], [[IN2]], [[IN3]]
  ...
  call void @llvm.evm.codecopy(ptr %dst, i256 %src, i256 %size)
  ret void
}
...
define void @extcodecopy(i256 %addr, ptr %dst, i256 %src, i256 %size) nounwind {
...
; CHECK: EXTCODECOPY [[IN1]], [[IN2]], [[IN3]], [[IN4]]
  ...
  call void @llvm.evm.extcodecopy(i256 %addr, ptr %dst, i256 %src, i256 %size)
  ret void
}
...
define void @returndatacopy(ptr %dst, i256 %src, i256 %size) nounwind {
...
; CHECK: RETURNDATACOPY [[IN1]], [[IN2]], [[IN3]]
  ...
  call void @llvm.evm.returndatacopy(ptr %dst, i256 %src, i256 %size)
  ret void
}
...
define void @log0(ptr %off, i256 %size) nounwind {
; CHECK-LABEL: @log0
; CHECK: ARGUMENT [[IN2:\$[0-9]+]], 1
; CHECK: ARGUMENT [[IN1:\$[0-9]+]], 0
; CHECK: LOG0 [[IN1]], [[IN2]]

  call void @llvm.evm.log0(ptr %off, i256 %size)
  ret void
}

define void @log1(ptr %off, i256 %size, i256 %t1) nounwind {
; CHECK-LABEL: @log1
; CHECK: ARGUMENT [[IN3:\$[0-9]+]], 2
; CHECK: ARGUMENT [[IN2:\$[0-9]+]], 1
; CHECK: ARGUMENT [[IN1:\$[0-9]+]], 0
; CHECK: LOG1 [[IN1]], [[IN2]], [[IN3]]
  ...
  call void @llvm.evm.log1(ptr %off, i256 %size, i256 %t1)
  ret void
}

define void @log2(ptr %off, i256 %size, i256 %t1, i256 %t2) nounwind {
; CHECK-LABEL: @log2
; CHECK: ARGUMENT [[IN4:\$[0-9]+]], 3
; CHECK: ARGUMENT [[IN3:\$[0-9]+]], 2
; CHECK: ARGUMENT [[IN2:\$[0-9]+]], 1
...
; CHECK: LOG2 [[IN1]], [[IN2]], [[IN3]], [[IN4]]
  ...
  call void @llvm.evm.log2(ptr %off, i256 %size, i256 %t1, i256 %t2)
  ret void
}

define void @log3(ptr %off, i256 %size, i256 %t1, i256 %t2, i256 %t3) nounwind {
; CHECK-LABEL: @log3
...
; CHECK: LOG3 [[IN1]], [[IN2]], [[IN3]], [[IN4]], [[IN5]]
  ...
  call void @llvm.evm.log3(ptr %off, i256 %size, i256 %t1, i256 %t2, i256 %t3)
  ret void
}

define void @log4(ptr %off, i256 %size, i256 %t1, i256 %t2, i256 %t3, i256 %t4) nounwind {
; CHECK-LABEL: @log4
...
; CHECK: LOG4 [[IN1]], [[IN2]], [[IN3]], [[IN4]], [[IN5]], [[IN6]]
  ...
  call void @llvm.evm.log4(ptr %off, i256 %size, i256 %t1, i256 %t2, i256 %t3, i256 %t4)
  ret void
}

define i256 @create(i256 %val, ptr %off, i256 %size) nounwind {
; CHECK-LABEL: @create
; CHECK: ARGUMENT [[IN3:\$[0-9]+]], 2
; CHECK: ARGUMENT [[IN2:\$[0-9]+]], 1
; CHECK: ARGUMENT [[IN1:\$[0-9]+]], 0
; CHECK: CREATE [[RES1:\$[0-9]+]], [[IN1]], [[IN2]], [[IN3]]
  ...
  %ret = call i256 @llvm.evm.create(i256 %val, ptr %off, i256 %size)
  ret i256 %ret
}

define i256 @call(i256 %gas, i256 %addr, i256 %val, ptr %arg_off, i256 %arg_size, ptr %ret_off, i256 %ret_size) nounwind {
; CHECK-LABEL: @call
...
; CHECK: CALL [[RES:\$[0-9]+]], [[IN1]], [[IN2]], [[IN3]], [[IN4]], [[IN5]], [[IN6]], [[IN7]]
  ...
  %ret = call i256 @llvm.evm.call(i256 %gas, i256 %addr, i256 %val, ptr %arg_off, i256 %arg_size, ptr %ret_off, i256 %ret_size)
  ret i256 %ret
}

define i256 @delegatecall(i256 %gas, i256 %addr, ptr %arg_off, i256 %arg_size, ptr %ret_off, i256 %ret_size) nounwind {
...
; CHECK: DELEGATECALL [[RES:\$[0-9]+]], [[IN1]], [[IN2]], [[IN3]], [[IN4]], [[IN5]], [[IN6]]
  ...
  %ret = call i256 @llvm.evm.delegatecall(i256 %gas, i256 %addr, ptr %arg_off, i256 %arg_size, ptr %ret_off, i256 %ret_size)
  ret i256 %ret
}

define i256 @create2(i256 %val, ptr %off, i256 %size, i256 %salt) nounwind {
; CHECK-LABEL: @create2
; CHECK: ARGUMENT [[IN4:\$[0-9]+]], 3
; CHECK: ARGUMENT [[IN3:\$[0-9]+]], 2
; CHECK: ARGUMENT [[IN2:\$[0-9]+]], 1
...
; CHECK: CREATE2 [[RES1:\$[0-9]+]], [[IN1]], [[IN2]], [[IN3]], [[IN4]]
  ...
  %ret = call i256 @llvm.evm.create2(i256 %val, ptr %off, i256 %size, i256 %salt)
  ret i256 %ret
}

define i256 @staticcall(i256 %gas, i256 %addr, ptr %arg_off, i256 %arg_size, ptr %ret_off, i256 %ret_size) nounwind {
...
; CHECK: STATICCALL [[RES:\$[0-9]+]], [[IN1]], [[IN2]], [[IN3]], [[IN4]], [[IN5]], [[IN6]]
  ...
  %ret = call i256 @llvm.evm.staticcall(i256 %gas, i256 %addr, ptr %arg_off, i256 %arg_size, ptr %ret_off, i256 %ret_size)
  ret i256 %ret
}
...
declare i256 @llvm.evm.sha3(ptr, i256)
...
declare void @llvm.evm.log0(ptr, i256)
declare void @llvm.evm.log1(ptr, i256, i256)
declare void @llvm.evm.log2(ptr, i256, i256, i256)
declare void @llvm.evm.log3(ptr, i256, i256, i256, i256)
declare void @llvm.evm.log4(ptr, i256, i256, i256, i256, i256)
declare i256 @llvm.evm.create(i256, ptr, i256)
declare i256 @llvm.evm.call(i256, i256, i256, ptr, i256, ptr, i256)
declare i256 @llvm.evm.delegatecall(i256, i256, ptr, i256, ptr, i256)
declare i256 @llvm.evm.create2(i256, ptr, i256, i256)
declare i256 @llvm.evm.staticcall(i256, i256, ptr, i256, ptr, i256)
