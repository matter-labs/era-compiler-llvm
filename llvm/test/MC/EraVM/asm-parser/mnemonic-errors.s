...
; far calls require special handling for .{st,sh} combined modifier
  far_call. {st,sh}    r1, r2, @foo ; space before the fixed sequence of tokens
  far_call.{st, sh}    r1, r2, @foo ; space inside the fixed sequence of tokens
  far_call.{sh,st}     r1, r2, @foo ; reversed order of "st" and "sh"
  far_call.lt.{st,sh}  r1, r2, @foo ; predicate before modifier
  far_call.{st}        r1, r2, @foo ; using with a single modifier
  far_call.static.{st,sh}  r1, r2, @foo ; multiple modifiers
  far_call.{st,sh}.static  r1, r2, @foo ; multiple modifiers
...
; CHECK-NEXT:    ^
; CHECK:       <stdin>:{{[0-9]+}}:13: error: no space allowed before or inside modifier
; CHECK-NEXT:    far_call. {st,sh}    r1, r2, @foo ; space before the fixed sequence of tokens
; CHECK-NEXT:              ^
; CHECK:       <stdin>:{{[0-9]+}}:17: error: no space allowed before or inside modifier
; CHECK-NEXT:    far_call.{st, sh}    r1, r2, @foo ; space inside the fixed sequence of tokens
; CHECK-NEXT:                  ^
; CHECK:       <stdin>:{{[0-9]+}}:13: error: '.{st,sh}' modifier expected
; CHECK-NEXT:    far_call.{sh,st}     r1, r2, @foo ; reversed order of "st" and "sh"
; CHECK-NEXT:              ^
; CHECK:       <stdin>:{{[0-9]+}}:15: error: cannot parse operand
; CHECK-NEXT:    far_call.lt.{st,sh}  r1, r2, @foo ; predicate before modifier
; CHECK-NEXT:                ^
; CHECK:       <stdin>:{{[0-9]+}}:15: error: '.{st,sh}' modifier expected
; CHECK-NEXT:    far_call.{st}        r1, r2, @foo ; using with a single modifier
; CHECK-NEXT:                ^
; CHECK:       <stdin>:{{[0-9]+}}:19: error: cannot parse operand
; CHECK-NEXT:    far_call.static.{st,sh}  r1, r2, @foo ; multiple modifiers
; CHECK-NEXT:                    ^
; CHECK:       <stdin>:{{[0-9]+}}:19: error: cannot parse operand
; CHECK-NEXT:    far_call.{st,sh}.static  r1, r2, @foo ; multiple modifiers
; CHECK-NEXT:                    ^
