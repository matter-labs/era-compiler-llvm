; RUN: llvm-mc -triple eravm -filetype=asm %s -o - | FileCheck %s

  .text
foo:

; mnemonics
  ld      r3, r4
  ; FIXME ld.inc  r3, r4
  uma.fat_ptr_read      r3, r4 ; aliased to "ld      r3, r4"
  ; FIXME uma.fat_ptr_read.inc  r3, r4 ; aliased to "ld.inc  r3, r4"

  ld.1      r3, r4
  ld.2      r3, r4
  ld.1.inc  r3, r4, r5
  ld.2.inc  r3, r4, r5
  uma.heap_read          r3, r4     ; aliased to "ld.1      r3, r4"
  uma.aux_heap_read      r3, r4     ; aliased to "ld.2      r3, r4"
  uma.inc.heap_read      r3, r4, r5 ; aliased to "ld.1.inc  r3, r4, r5"
  uma.inc.aux_heap_read  r3, r4, r5 ; aliased to "ld.2.inc  r3, r4, r5"

  st.1      r3, r4
  st.2      r3, r4
  st.1.inc  r3, r4, r5
  st.2.inc  r3, r4, r5
  uma.heap_write          r3, r4     ; aliased to "st.1      r3, r4"
  uma.aux_heap_write      r3, r4     ; aliased to "st.2      r3, r4"
  uma.inc.heap_write      r3, r4, r5 ; aliased to "st.1.inc  r3, r4, r5"
  uma.inc.aux_heap_write  r3, r4, r5 ; aliased to "st.2.inc  r3, r4, r5"

; modifiers (condition code)

  ld.lt      r3, r4
  ; FIXME ld.inc.lt  r3, r4
  ld.1.lt      r3, r4
  ld.2.lt      r3, r4
  ld.1.inc.lt  r3, r4, r5
  ld.2.inc.lt  r3, r4, r5
  st.1.lt      r3, r4
  st.2.lt      r3, r4
  st.1.inc.lt  r3, r4, r5
  st.2.inc.lt  r3, r4, r5

; operands (instructions operating on "local" heap can accept immediate addresses)
  ld.1      42, r4
  ld.2      42, r4
  ; FIXME ld.1.inc  42, r4, r5
  ; FIXME ld.2.inc  42, r4, r5
  st.1      42, r4
  st.2      42, r4
  ; FIXME st.1.inc  42, r4, r5
  ; FIXME st.2.inc  42, r4, r5

; COM: Autogenerated checks below, see README.md.
; CHECK:  .text
; CHECK:foo:

; CHECK:  ld   r3, r4
; CHECK:  ld   r3, r4

; CHECK:  ld.1 r3, r4
; CHECK:  ld.2 r3, r4
; CHECK:  ld.1.inc     r3, r4, r5
; CHECK:  ld.2.inc     r3, r4, r5
; CHECK:  ld.1 r3, r4
; CHECK:  ld.2 r3, r4
; CHECK:  ld.1.inc     r3, r4, r5
; CHECK:  ld.2.inc     r3, r4, r5

; CHECK:  st.1 r3, r4
; CHECK:  st.2 r3, r4
; CHECK:  st.1.inc     r3, r4, r5
; CHECK:  st.2.inc     r3, r4, r5
; CHECK:  st.1 r3, r4
; CHECK:  st.2 r3, r4
; CHECK:  st.1.inc     r3, r4, r5
; CHECK:  st.2.inc     r3, r4, r5


; CHECK:  ld.lt        r3, r4
; CHECK:  ld.1.lt      r3, r4
; CHECK:  ld.2.lt      r3, r4
; CHECK:  ld.1.inc.lt  r3, r4, r5
; CHECK:  ld.2.inc.lt  r3, r4, r5
; CHECK:  st.1.lt      r3, r4
; CHECK:  st.2.lt      r3, r4
; CHECK:  st.1.inc.lt  r3, r4, r5
; CHECK:  st.2.inc.lt  r3, r4, r5

; CHECK:  ld.1 42, r4
; CHECK:  ld.2 42, r4
; CHECK:  st.1 42, r4
; CHECK:  st.2 42, r4
