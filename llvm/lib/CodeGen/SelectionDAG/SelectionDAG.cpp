...
SDValue SelectionDAG::getConstant(const ConstantInt &Val, const SDLoc &DL,
  ...
  // EraVM local begin
  // nullptrs in address space 3 can be generated by the compiler, but fatptr
  // is not and integer type. To handle the issue create a dag node with i256
  // type.
  if (VT == MVT::fatptr)
    VT = MVT::i256;
  // EraVM local end
...
MaybeAlign SelectionDAG::InferPtrAlign(SDValue Ptr) const {
  ...
  if (FrameIndexSDNode *FI = dyn_cast<FrameIndexSDNode>(Ptr)) {
    ...
    // Though the architecture guarantees the offset fits 64 bits wide
    // signed integer, so it's ok to truncate.
    if (Ptr.getOperand(1).getSimpleValueType() == MVT::i256)
