//===------ EraVMEHPrepare - Prepare excepton handling for EraVM --------===//
//
// This transformation is designed for use by code generators which use
// EraVM exception handling scheme.
// The implementation is inherited from WebAssembly.
//
// At the moment EraVM only supports limited capacity for exception handling:
...
/// pad instruction.
class EraVMEHPrepare : public FunctionPass {
  Function *ThrowF = nullptr; // eravm.throw() intrinsic
  ...
  /// longer valid, nor they are needed since EraVM does not allow to
  ...
  /// __cxa_trow always throws and EraVM does not differentiate between
  /// excaptions.
...
public:
  ...
  EraVMEHPrepare() : FunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return "EraVM Exception handling preparation";
  }
};
} // end anonymous namespace

char EraVMEHPrepare::ID = 0;
INITIALIZE_PASS_BEGIN(EraVMEHPrepare, DEBUG_TYPE, "Prepare EraVM exceptions",
                      false, false)
INITIALIZE_PASS_DEPENDENCY(DominatorTreeWrapperPass)
INITIALIZE_PASS_END(EraVMEHPrepare, DEBUG_TYPE, "Prepare EraVM exceptions",
                    false, false)

FunctionPass *llvm::createEraVMEHPass() { return new EraVMEHPrepare(); }
...
bool EraVMEHPrepare::doInitialization(Module &M) {
...
bool EraVMEHPrepare::runOnFunction(Function &F) {
  bool Changed = false;
...
bool EraVMEHPrepare::replaceCXAThrow(Function &F) {
  bool Changed = false;
  Module &M = *F.getParent();
  ThrowF = Intrinsic::getDeclaration(&M, Intrinsic::eravm_throw);
  for (auto &BB : F)
...
bool EraVMEHPrepare::replaceInvokeFunction(Function &F) {
  bool Changed = false;
  for (BasicBlock &BB : F)
    if (InvokeInst *II = dyn_cast<InvokeInst>(BB.getTerminator())) {
      ...
      auto *Fn =
          Intrinsic::getDeclaration(F.getParent(), Intrinsic::eravm_ltflag);
...
bool EraVMEHPrepare::replaceInvokeIntrinsic(Function &F) {
  bool Changed = false;
  Module &M = *F.getParent();
  ThrowF = Intrinsic::getDeclaration(&M, Intrinsic::eravm_throw);
  for (auto &BB : F)
    for (auto II = BB.begin(); II != BB.end(); ++II) {
      ...
      if (!CallSite->isIntrinsic())
        continue;
      if (CallSite->getIntrinsicID() != Intrinsic::eravm_sstore)
        continue;
      IRBuilder<> Builder(&Inst);
...
bool EraVMEHPrepare::prepareThrows(Function &F) {
  ...
  // eravm.throw() intinsic, which will be lowered to eravm 'throw'
  // instruction.
  ThrowF = Intrinsic::getDeclaration(&M, Intrinsic::eravm_throw);
  // Insert an unreachable instruction after a call to @llvm.eravm.throw and
  // delete all following instructions within the BB, and delete all the dead
  // children of the BB as well.
  for (User *U : ThrowF->users()) {
    // A call to @llvm.eravm.throw() can not be an InvokeInst.
...
bool EraVMEHPrepare::removeLPad(Function &F) {
