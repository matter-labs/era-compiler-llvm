//===-- SHA3ConstFolding.cpp - Const fold calls to sha3 ---------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// The code below tries to replace sha3 (that calucates keccak256 hash) calls
// with the calculated hash values.
//
// It uses the following general approach: given a sha3 call (MemoryUse),
...
// Only sha3 calls with the constant 'size' argument are checked.
//
// For example:
//
//   store i256 1, ptr addrspace(1) null
//   store i256 2, ptr addrspace(1) inttoptr (i256 32 to ptr addrspace(1))
//   %hash = tail call i256 @sha3(ptr addrspace(1) null, i256 64, i1 true)
//   ret i256 %hash
//
// is transformed into:
//
...
//
// 2. Check that
//   1. Each clobber is withing the sha3 memory location:
//        |--clobber--|
//     |------MemUse-------|
...
//    the Keccak256 hash.
//
// 4. Run simplification for each instruction in the function, as sha3 folding
...
#define DEBUG_TYPE "sha3-constant-folding"

STATISTIC(NumSHA3Folded, "Number of sha3 calls folded");

DEBUG_COUNTER(SHA3Counter, "sha3-constant-folding",
              "Controls which instructions are removed");

namespace {
/// This structure holds an information about a single memory clobber.
/// While walking upwards starting at a sha3 call (which is a MemUse),
...
/// representation of a continuous memory region the sha3 computes a hash for.
struct MemClobber {
  // Byte offset relatively to the beginning of the sha3 memory location.
  uint64_t Start;
...
/// This class holds an information required for folding sha3 calls
/// in the function F.
///
...
public:
  ...
  /// given sha3 call (\p Call).
  SmallVector<MemoryDef *, 8> collectSHA3Clobbers(const CallInst *Call);

  /// For the given sha3 call (\p Call), walk through the collected MemorySSA
  ...
  /// simplification may appear after replacing sha3 calls with the
  ...
  /// Return collected MemorySSa uses corresponding to sha3 calls.
...
private:
  ...
  bool shouldSkipClobber(const Instruction *MemInst) const {
    ...
    if (const auto *Intr = dyn_cast<AnyMemIntrinsic>(MemInst))
      return Intr->getDestAddressSpace() != HeapAS;
...
FoldingState::FoldingState(
    ...
    const std::function<bool(const Instruction *)> &IsSha3Call, unsigned HeapAS)
  ...
  for (BasicBlock *BB : RPOT) {
    for (Instruction &I : *BB) {
      ...
      // sha3 is passed a pointer in the first argument
...
FoldingState::collectSHA3Clobbers(const CallInst *Call) {
  ...
  // For the given sha3 call (which is MemoryUse in terms of MemorySSA) we
  ...
  //  - live on entry (nothing to do, sha3 is not clobbered),
  //  - MemoryDef,
  //  - MemoryPhi
  //
  // We start with a nearest to the sha3 call dominating clobbering access.
...
bool FoldingState::simplifyInstructions() {
  ...
  for (auto &Inst : make_early_inc_range(instructions(F))) {
    if (Value *V = simplifyInstruction(&Inst, SQ)) {
      LLVM_DEBUG(dbgs() << " SHA3ConstFolding simplify: " << Inst
                        << "  to: " << *V << '\n');
...
Value *FoldingState::runFolding(const CallInst *Call,
  ...
  for (MemoryDef *MemDef : MemDefs) {
    ...
    // This clobber doesn't write to the memory sha3 is reading from.
    ...
    // to determine the full memory data for the sha3.
    ...
    // If we have collected clobbers that fully cover the sha3 memory
...
bool llvm::runSHA3ConstFolding(
    Function &F, AliasAnalysis &AA, AssumptionCache &AC, MemorySSA &MSSA,
    DominatorTree &DT, const TargetLibraryInfo &TLI, const LoopInfo &LI,
    const std::function<bool(const Instruction *)> &IsSha3Call,
    unsigned HeapAS) {
  LLVM_DEBUG(dbgs() << "********** SHA3 constant folding **********\n"
                    << "********** Function: " << F.getName() << '\n');

  FoldingState State(F, AA, AC, MSSA, DT, TLI, LI, IsSha3Call, HeapAS);
  ...
  do {
    ...
    // If we simplified some instructions after folding sha3 calls,
