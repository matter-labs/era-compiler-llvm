//===------------- MergeIdenticalBB.cpp - Identical BB merging pass
//-----------===//
//
...
static bool canMergeBlocks(const BasicBlock &BB1, const BasicBlock &BB2) {
  ...
  if (!std::equal(
          succ_begin(&BB1), succ_end(&BB1), succ_begin(&BB2),
...
static bool mergeIdenticalBBImpl(Function &F, const TargetTransformInfo &TTI,
                                 DominatorTree *DT) {
...
static bool mergeIdenticalBB(Function &F, const TargetTransformInfo &TTI,
                             DominatorTree *DT) {
  assert((!RequireAndPreserveDomTree ||
          (DT && DT->verify(DominatorTree::VerificationLevel::Full))) &&
         "Original domtree is invalid?");

  bool Changed = mergeIdenticalBBImpl(F, TTI, DT);
...
PreservedAnalyses MergeIdenticalBBPass::run(Function &F,
                                            FunctionAnalysisManager &AM) {
  auto &TTI = AM.getResult<TargetIRAnalysis>(F);
  DominatorTree *DT = nullptr;
  if (RequireAndPreserveDomTree)
    DT = &AM.getResult<DominatorTreeAnalysis>(F);
  if (!mergeIdenticalBB(F, TTI, DT))
