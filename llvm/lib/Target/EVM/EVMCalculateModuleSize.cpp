...
//===----------------------------------------------------------------------===//
//
// This pass calculates size of a whole module.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "evm-calculate-module-size"
#define PASS_NAME "EVM calculate module size"

namespace {
class EVMCalculateModuleSize : public ModulePass {
public:
  static char ID;

  EVMCalculateModuleSize() : ModulePass(ID) {
    initializeEVMCalculateModuleSizePass(*PassRegistry::getPassRegistry());
  }
...
char EVMCalculateModuleSize::ID = 0;

INITIALIZE_PASS_BEGIN(EVMCalculateModuleSize, DEBUG_TYPE, PASS_NAME, false,
                      false)
INITIALIZE_PASS_DEPENDENCY(MachineModuleInfoWrapperPass)
INITIALIZE_PASS_END(EVMCalculateModuleSize, DEBUG_TYPE, PASS_NAME, false, false)

ModulePass *llvm::createEVMCalculateModuleSize() {
  return new EVMCalculateModuleSize();
}
...
static unsigned getInstSize(const MachineInstr &MI,
  ...
  case EVM::PseudoJUMP:
    // Even though we are emitting PUSH4_S, most probably this will be relaxed
    // to PUSH2_S when generating object code. In case we need larger
    // instruction, bytecode limit is exceeded and this doesn't matter.
...
static uint64_t calculateFunctionCodeSize(const MachineFunction &MF) {
  uint64_t Size = 0;
...
bool EVMCalculateModuleSize::runOnModule(Module &M) {
  LLVM_DEBUG({ dbgs() << "********** Calculate module size **********\n"; });
  ...
  for (Function &F : M) {
    ...
    if (!MF)
      continue;
    TotalSize += calculateFunctionCodeSize(*MF);
  }

  // Take into account INVALID instruction at the end of the .text section.
  TotalSize++;
  dbgs() << "Total module size: " << TotalSize << " bytes\n";
  return false;
}
