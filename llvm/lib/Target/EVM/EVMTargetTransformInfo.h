...
public:
  ...
  TypeSize
  getRegisterBitWidth(TargetTransformInfo::RegisterKind RK) const override {
    assert(RK == TargetTransformInfo::RGK_Scalar &&
           "Vector registers aren't supported");
    return TypeSize::getFixed(256);
  }

  unsigned getRegisterClassForType(bool IsVector,
                                   Type *Ty = nullptr) const override {
    return IsVector ? Vector : GPR;
  }

  const char *getRegisterClassName(unsigned ClassID) const override {
    assert(ClassID == GPR && "Unknown register class");
    return "EVM::GPR";
  }
