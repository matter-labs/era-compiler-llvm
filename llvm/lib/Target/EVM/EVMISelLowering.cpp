...
SDValue EVMTargetLowering::lowerINTRINSIC_WO_CHAIN(SDValue Op,
  ...
  case Intrinsic::evm_datasizeruntime:
...
SDValue EVMTargetLowering::lowerIntrinsicDataSize(unsigned IntrID, SDValue Op,
  ...
  MCSymbol *Sym = MF.getContext().getOrCreateSymbol(
      IsDatasize ? "__evm_datasize" : "__evm_datasize_runtime");
  return SDValue(
      DAG.getMachineNode(IsDatasize ? EVM::DATASIZE : EVM::DATASIZE_RUNTIME, DL,
                         Ty, DAG.getMCSymbol(Sym, MVT::i256)),
      0);
}
