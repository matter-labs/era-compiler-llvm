...
public:
  ...
  /// Requires @a Cfg to be a control flow graph generated from disambiguated
  /// Yul. If @a _functionName is empty, the stack too deep errors of the main
  /// entry point are reported instead.
...
private:
  ...
  /// @returns the optimal entry stack layout, s.t. @a Operation can be applied
  /// to it and the result can be transformed to @a ExitStack with minimal stack
  ...
  /// @returns the desired stack layout at the entry of @a Block, assuming the
  /// layout after executing the block should be @a ExitStack.
  ...
  /// @returns the best known exit layout of @a Block, if all dependencies are
  /// already @a Visited. If not, adds the dependencies to @a DependencyList and
  /// @returns std::nullopt.
  ...
  /// in the graph starting at @a Eentry.
  ...
  /// Calculates the ideal stack layout, s.t. both @a _stack1 and @a _stack2 can
  ...
  /// @returns a copy of @a Stack stripped of all duplicates and slots that can
  /// be freely generated. Attempts to create a layout that requires a minimal
  /// amount of operations to reconstruct the original stack @a Stack.
  ...
  static std::string Test(MachineBasicBlock *MBB);
};
...
public:
  ...
  void operator()(CFG::FunctionInfo const &_info);

private:
  void printBlock(CFG::BasicBlock const &_block);
  std::string getBlockId(CFG::BasicBlock const &_block);
