...
bool EVMELFObjectWriter::needsRelocateWithSymbol(const MCSymbol &Sym,
                                                 unsigned Type) const {
  // currently we spoort only relocations with symbol, not section
  return true;
}
...
unsigned EVMELFObjectWriter::getRelocType(MCContext &Ctx, const MCValue &Target,
  ...
  case EVM::fixup_DataSize_i64:
    return ELF::R_EVM_DATASIZE;
  case EVM::fixup_DataSize_Runtime_i64:
    return ELF::R_EVM_DATASIZE_RUNTIME;
  }
}
