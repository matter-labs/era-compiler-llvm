//===------ EraVMCombineInstrs.cpp - Combine instructions Pre-RA ---------===//
//
/// \file
/// This file contains a pass that combines instructions.
/// It relies on def-use chains and runs on pre-RA phase.
///
/// Similar to ARM architecture, arithmetic instructions in EraVM has the
...
#define DEBUG_TYPE "eravm-combine-instrs"
#define ERAVM_COMBINE_INSTRUCTIONS_NAME "EraVM combine instructions"

static cl::opt<bool> EnableEraVMCombine("enable-eravm-combine-instrs",
                                        cl::init(true), cl::Hidden);

namespace {

class EraVMCombineInstrs : public MachineFunctionPass {
public:
  static char ID;
  EraVMCombineInstrs() : MachineFunctionPass(ID) {}

  bool runOnMachineFunction(MachineFunction &Fn) override;

  StringRef getPassName() const override {
    return ERAVM_COMBINE_INSTRUCTIONS_NAME;
  }
...
char EraVMCombineInstrs::ID = 0;

} // namespace

INITIALIZE_PASS(EraVMCombineInstrs, DEBUG_TYPE, ERAVM_COMBINE_INSTRUCTIONS_NAME,
                false, false)

bool EraVMCombineInstrs::runOnMachineFunction(MachineFunction &MF) {
  LLVM_DEBUG(dbgs() << "********** EraVM COMBINE INSTRUCTIONS **********\n"
                    << "********** Function: " << MF.getName() << '\n');

  if (!EnableEraVMCombine) {
  ...
  for (MachineBasicBlock &MBB : MF)
    for (auto MI = MBB.begin(); MI != MBB.end(); ++MI) {
      ...
      if (MI->getOpcode() == EraVM::SUBxrr_v) {
        ...
        auto hasNoFlagsDefOrUseBetween = [&](auto Start, auto End) {
          ...
          for (auto I = std::next(Start); I != End; ++I) {
            ...
            if (TII->isFlagSettingInstruction(I->getOpcode()) ||
                I->getOpcode() == EraVM::ADDframe || I->isCall()) {
        ...
        if (llvm::EraVM::getFlagSettingOpcode(DefMI->getOpcode()) == -1) {
        ...
        DefMI->setDesc(
            TII->get(EraVM::getFlagSettingOpcode(DefMI->getOpcode())));
...
/// createEraVMCombineInstrsPass - returns an instance of the pseudo
/// instruction expansion pass.
FunctionPass *llvm::createEraVMCombineInstrsPass() {
  return new EraVMCombineInstrs();
}
