//===------ EraVMIndirectUMA.cpp - Wrap an UMA into a function call ------===//
...
/// UMA is a very expensive operation in EraVM, so to prevent code from being
...
struct EraVMIndirectUMA : public ModulePass {
public:
  static char ID;
  EraVMIndirectUMA() : ModulePass(ID) {
    initializeEraVMIndirectUMAPass(*PassRegistry::getPassRegistry());
  }
...
INITIALIZE_PASS(EraVMIndirectUMA, "eravm-indirectuma",
                "Wrap an UMA into a function call", false, false)

bool EraVMIndirectUMA::runOnModule(Module &M) {
  bool Changed = false;
...
ModulePass *llvm::createEraVMIndirectUMAPass() {
  return new EraVMIndirectUMA();
}
