//===--- SyncVMCombineAddressingMode.cpp - use complex addressing modes ---===//
//
/// \file
...
#define SYNCVM_COMBINE_ADDRESSING_MODE_NAME                                    \
  "SyncVM combine instuctions to use complex addressing modes"
...
private:
  ...
  void replaceArgument(MachineInstr &Base, SyncVM::ArgumentKind ArgNo,
...
bool SyncVMCombineAddressingMode::isReloadInst(const MachineInstr &MI) const {
...
bool SyncVMCombineAddressingMode::isSpillInst(const MachineInstr &MI) const {
  return (MI.getOpcode() == SyncVM::ADDrrs_s ||
          MI.getOpcode() == SyncVM::PTR_ADDrrs_s) &&
         !(SyncVM::out0Iterator(MI) + 1)->isReg() &&
         SyncVM::in1Iterator(MI)->getReg() == SyncVM::R0 &&
         TII->getCCCode(MI) == SyncVMCC::COND_NONE;
}

void SyncVMCombineAddressingMode::replaceArgument(
    MachineInstr &Base, SyncVM::ArgumentKind ArgNo,
    iterator_range<MachineInstr::const_mop_iterator> NewArg,
    unsigned NewOpcode) {
  assert(ArgNo == SyncVM::ArgumentKind::In0 ||
         ArgNo == SyncVM::ArgumentKind::In1);
  auto NewMI =
      BuildMI(*Base.getParent(), Base, Base.getDebugLoc(), TII->get(NewOpcode));
  SyncVM::copyOperands(NewMI, Base.operands_begin(), SyncVM::in0Iterator(Base));
  SyncVM::copyOperands(NewMI, NewArg);
  SyncVM::copyOperands(NewMI, (ArgNo == SyncVM::ArgumentKind::In1)
                                  ? SyncVM::in0Range(Base)
                                  : SyncVM::in1Range(Base));
  SyncVM::copyOperands(NewMI, SyncVM::in1Iterator(Base) + 1,
                       Base.operands_begin() + Base.getNumExplicitOperands());
}

void SyncVMCombineAddressingMode::mergeSelect(
    MachineInstr &Base, MachineInstr &Def,
    iterator_range<MachineInstr::const_mop_iterator> In) {
  // TODO: CPR-1131 Allow to merge twice.
  assert(TII->isSel(Base) && "Expected Select instruction");
  auto In0It = SyncVM::in0Iterator(Base);
  bool IsIn0 =
      In0It->isReg() && In0It->getReg() == SyncVM::out0Iterator(Def)->getReg();
  ...
  SyncVM::copyOperands(NewMI, Base.operands_begin(), SyncVM::in0Iterator(Base));
  if (IsIn0)
    SyncVM::copyOperands(NewMI, In);
  else
    SyncVM::copyOperands(NewMI, SyncVM::in0Range(Base));
  if (!IsIn0)
    SyncVM::copyOperands(NewMI, In);
  else
    SyncVM::copyOperands(NewMI, SyncVM::in1Iterator(Base),
                         Base.operands_begin() + Base.getNumExplicitOperands() -
                             1);
...
bool SyncVMCombineAddressingMode::canCombineUse(
  ...
  if (TII->isSel(Use)) {
    const MachineOperand &FirstOpnd = *SyncVM::in0Iterator(Use);
    const MachineOperand &SecondOpnd = *SyncVM::in1Iterator(Use);
    ...
    if (std::any_of(std::next(Reload.getIterator()), Use.getIterator(),
                    [&Use](const MachineInstr &CurMI) {
                      if (!SyncVM::hasSRInAddressingMode(CurMI))
                        return false;
                      return areEqualStackSlots(SyncVM::in0Iterator(CurMI),
  ...
  if (SyncVM::in0Iterator(Use)->getReg() == Def &&
    ...
    return true;
  if ((Use.isCommutable() ||
       SyncVM::getWithInsSwapped(
           SyncVM::getWithSRInAddrMode(Use.getOpcode())) != -1) &&
...
bool SyncVMCombineAddressingMode::canCombineDef(
  ...
  if (getImmOrCImm(*SyncVM::ccIterator(Def)) != SyncVMCC::COND_NONE)
    return false;
  ...
  if (std::any_of(std::next(Def.getIterator()), Spill.getIterator(),
                  [&Spill](const MachineInstr &CurMI) {
                    ...
                    if (SyncVM::hasSROutAddressingMode(CurMI))
                      ClobberedStackSlot |=
                          areEqualStackSlots(SyncVM::out0Iterator(Spill),
                                             SyncVM::out0Iterator(CurMI));
                    if (SyncVM::hasSRInAddressingMode(CurMI))
                      ClobberedStackSlot |=
                          areEqualStackSlots(SyncVM::out0Iterator(Spill),
...
bool SyncVMCombineAddressingMode::combineReloadUse(MachineFunction &MF) {
  ...
  for (auto &MBB : MF) {
    for (auto &MI : MBB) {
      ...
      if (!all_of(Uses, [this, &MI](const MachineInstr *Use) {
            return canCombineUse(MI, *Use, SyncVM::in0Iterator(MI));
          }))
        continue;
  ...
  for (auto [Reload, Uses] : Deleted) {
    for (auto Use : Uses) {
      if (SyncVM::isSelect(*Use)) {
        mergeSelect(*Use, *Reload, SyncVM::in0Range(*Reload));
      } else {
        if (SyncVM::in0Iterator(*Use)->getReg() ==
            Reload->getOperand(0).getReg()) {
          replaceArgument(*Use, SyncVM::ArgumentKind::In0,
                          SyncVM::in0Range(*Reload),
                          SyncVM::getWithSRInAddrMode(Use->getOpcode()));
        } else {
          int NewOpcode = SyncVM::getWithSRInAddrMode(Use->getOpcode());
          if (!Use->isCommutable())
            NewOpcode = SyncVM::getWithInsSwapped(NewOpcode);
          assert(NewOpcode != -1);
          replaceArgument(*Use, SyncVM::ArgumentKind::In1,
                          SyncVM::in0Range(*Reload), NewOpcode);
        }
      }
...
bool SyncVMCombineAddressingMode::combineDefSpill(MachineFunction &MF) {
  ...
  for (auto &MBB : MF) {
    for (auto &MI : MBB) {
      if (!isSpillInst(MI))
        continue;
      Register Spilled = SyncVM::in0Iterator(MI)->getReg();
      SmallPtrSet<MachineInstr *, 4> ReachingDefs;
      ...
      if (!all_of(DefUses, [&MI, DefMI, &UsesToUpdate,
            ...
            if (DefUse == &MI)
              return true;
            return canCombineUse(*DefMI, *DefUse, SyncVM::in0Iterator(MI), &MI);
          }))
        continue;
  ...
  for (auto [Def, Spill, Uses] : Deleted) {
    ...
    // remove the spill.
    int NewOpcode = SyncVM::getWithSROutAddrMode(Def->getOpcode());
    assert(NewOpcode != -1);
    MachineInstrBuilder NewMI;
    if (Def->getNumExplicitDefs() == 2u)
      NewMI =
          BuildMI(*Def->getParent(), *Def, Def->getDebugLoc(),
                  TII->get(NewOpcode), SyncVM::out1Iterator(*Def)->getReg());
    else
      NewMI = BuildMI(*Def->getParent(), *Def, Def->getDebugLoc(),
                      TII->get(NewOpcode));

    SyncVM::copyOperands(NewMI, SyncVM::in0Iterator(*Def),
                         SyncVM::in1Iterator(*Def) + 1);
    SyncVM::copyOperands(NewMI, SyncVM::out0Range(*Spill));
    SyncVM::copyOperands(NewMI, SyncVM::in1Iterator(*Def) + 1,
    ...
    for (MachineInstr *Use : Uses) {
      ...
      if (Use == Spill)
        continue;
      if (TII->isSel(*Use)) {
        mergeSelect(*Use, *Def, SyncVM::out0Range(*Spill));
      } else {
        if (SyncVM::in0Iterator(*Use)->getReg() == Def->getOperand(0).getReg())
          replaceArgument(*Use, SyncVM::ArgumentKind::In0,
                          SyncVM::out0Range(*Spill),
                          SyncVM::getWithSRInAddrMode(Use->getOpcode()));
        else {
          int NewOpcode = SyncVM::getWithSRInAddrMode(Use->getOpcode());
          if (!Use->isCommutable())
          ...
          assert(NewOpcode != -1);
          replaceArgument(*Use, SyncVM::ArgumentKind::In1,
                          SyncVM::out0Range(*Spill), NewOpcode);
        }
      }
