...
static void fixupStackOffsetPostOutline(MachineBasicBlock &MBB,
  ...
  auto AdjustDisp = [FixupOffset](MachineInstr::mop_iterator It) {
    if (!It->isReg() || It->getReg() != SyncVM::SP)
      return;
...
void SyncVMInstrInfo::fixupPostOutline(MachineFunction &MF) const {
  ...
  for (MachineBasicBlock &MBB : MF)
    fixupStackOffsetPostOutline(MBB, StackOffset);
...
bool SyncVMInstrInfo::isFunctionSafeToOutlineFrom(
  ...
  auto InvalidStackRelativeAM = [](MachineInstr::mop_iterator It) {
    return It->isReg() && It->getReg() == SyncVM::SP && !(It + 2)->isImm();
  };
  ...
  for (MachineBasicBlock &MBB : MF) {
    if (any_of(instructionsWithoutDebug(MBB.begin(), MBB.end()),
               [InvalidStackRelativeAM](MachineInstr &MI) {
                 if (SyncVM::hasSRInAddressingMode(MI) &&
                     InvalidStackRelativeAM(SyncVM::in0Iterator(MI)))
                   return true;
                 if (SyncVM::hasSROutAddressingMode(MI) &&
                     InvalidStackRelativeAM(SyncVM::out0Iterator(MI)))
                   return true;
                 return false;
               }))
      return false;
...
outliner::OutlinedFunction SyncVMInstrInfo::getOutliningCandidateInfo(
  ...
  for (auto &C : RepeatedSequenceLocs) {
    ...
    MachineFunction *MF = C.front()->getMF();
...
void SyncVMInstrInfo::buildOutlinedFrame(
  ...
  outliner::Candidate &FirstCand =
      const_cast<outliner::OutlinedFunction &>(OF).Candidates.front();
