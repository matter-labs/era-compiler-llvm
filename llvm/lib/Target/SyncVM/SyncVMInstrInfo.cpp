...
static int64_t getCC(const MachineOperand &cond) {
  if (cond.isImm()) {
    return cond.getImm();
  }
  if (cond.isCImm()) {
    int64_t imm = cond.getCImm()->getZExtValue();
    return imm;
  }
  llvm_unreachable ("Invalid CC type.");
}
...
bool SyncVMInstrInfo::reverseBranchCondition(
  ...
  SyncVMCC::CondCodes CC = static_cast<SyncVMCC::CondCodes>(getCC(Cond[0]));
...
bool SyncVMInstrInfo::analyzeBranch(MachineBasicBlock &MBB,
  ...
  while (I != MBB.begin()) {
    ...
    SyncVMCC::CondCodes BranchCode =
      static_cast<SyncVMCC::CondCodes>(getCC(I->getOperand(1)));
...
unsigned SyncVMInstrInfo::insertBranch(
  ...
  // Conditional branch.
  BuildMI(&MBB, DL, get(SyncVM::J)).addMBB(TBB).addImm(getCC(Cond[0]));
  ++Count;
