//===-- SyncVMAliasAnalysis.cpp - SyncVM alias analysis -------------------===//
//
...
#define DEBUG_TYPE "syncvm-aa"

AnalysisKey SyncVMAA::Key;

// Register this pass...
char SyncVMAAWrapperPass::ID = 0;
char SyncVMExternalAAWrapper::ID = 0;

INITIALIZE_PASS(SyncVMAAWrapperPass, "syncvm-aa",
                "SyncVM Address space based Alias Analysis", false, true)

INITIALIZE_PASS(SyncVMExternalAAWrapper, "syncvm-aa-wrapper",
                "SyncVM Address space based Alias Analysis Wrapper", false,
                true)

ImmutablePass *llvm::createSyncVMAAWrapperPass() {
  return new SyncVMAAWrapperPass();
}

ImmutablePass *llvm::createSyncVMExternalAAWrapperPass() {
  return new SyncVMExternalAAWrapper();
}

SyncVMAAWrapperPass::SyncVMAAWrapperPass() : ImmutablePass(ID) {
  initializeSyncVMAAWrapperPassPass(*PassRegistry::getPassRegistry());
}

void SyncVMAAWrapperPass::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.setPreservesAll();
}
...
AliasResult SyncVMAAResult::alias(const MemoryLocation &LocA,
  ...
  if (ASA > SyncVMAS::MAX_ADDRESS || ASB > SyncVMAS::MAX_ADDRESS)
