...
SyncVMTargetLowering::SyncVMTargetLowering(const TargetMachine &TM,
  ...
  addRegisterClass(MVT::i8,  &SyncVM::GR8RegClass);
  ...
  if (STI.hasHWMult16()) {
    ...
    setLibcallCallingConv(RTLIB::MUL_I64, CallingConv::SyncVM_BUILTIN);
  }

  // Several of the runtime library functions use a special calling conv
  setLibcallCallingConv(RTLIB::UDIV_I64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::UREM_I64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::SDIV_I64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::SREM_I64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::ADD_F64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::SUB_F64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::MUL_F64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::DIV_F64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::OEQ_F64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::UNE_F64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::OGE_F64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::OLT_F64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::OLE_F64, CallingConv::SyncVM_BUILTIN);
  setLibcallCallingConv(RTLIB::OGT_F64, CallingConv::SyncVM_BUILTIN);
...
bool SyncVMTargetLowering::shouldAvoidTransformToShift(EVT VT,
                                                       unsigned Amount) const {
  return !(Amount == 8 || Amount == 9 || Amount<=2);
}

// Implemented to verify test case assertions in
// tests/codegen/syncvm/shift-amount-threshold-b.ll
bool SyncVMTargetLowering::isLegalICmpImmediate(int64_t Immed) const {
  if (SyncVMNoLegalImmediate)
    return Immed >= -32 && Immed < 32;
  return TargetLowering::isLegalICmpImmediate(Immed);
}

//===----------------------------------------------------------------------===//
//                       SyncVM Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
TargetLowering::ConstraintType
SyncVMTargetLowering::getConstraintType(StringRef Constraint) const {
...
SyncVMTargetLowering::getRegForInlineAsmConstraint(
  ...
  if (Constraint.size() == 1) {
    ...
    case 'r':   // GENERAL_REGS
      if (VT == MVT::i8)
        return std::make_pair(0U, &SyncVM::GR8RegClass);

      return std::make_pair(0U, &SyncVM::GR16RegClass);
    }
  }
...
static void AnalyzeVarArgs(CCState &State,
  ...
  State.AnalyzeCallOperands(Outs, CC_SyncVM_AssignStack);
}

static void AnalyzeVarArgs(CCState &State,
                           const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeFormalArguments(Ins, CC_SyncVM_AssignStack);
}
...
static void AnalyzeArguments(CCState &State,
  ...
  bool Builtin = (State.getCallingConv() == CallingConv::SyncVM_BUILTIN);
  if (Builtin) {
  ...
  for (unsigned i = 0, e = ArgsParts.size(); i != e; i++) {
    ...
    if (!UsedStack && Parts == 2 && RegsLeft == 1) {
      ...
      UsedStack = true;
      CC_SyncVM_AssignStack(ValNo++, ArgVT, LocVT, LocInfo, ArgFlags, State);
...
static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::InputArg> &Ins) {
  State.AnalyzeCallResult(Ins, RetCC_SyncVM);
}

static void AnalyzeRetResult(CCState &State,
                             const SmallVectorImpl<ISD::OutputArg> &Outs) {
  State.AnalyzeReturn(Outs, RetCC_SyncVM);
}
...
SyncVMTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // SyncVM target does not yet support tail call optimization.
  isTailCall = false;

  switch (CallConv) {
  default:
    report_fatal_error("Unsupported calling convention");
  case CallingConv::SyncVM_BUILTIN:
  case CallingConv::Fast:
  case CallingConv::C:
...
SDValue SyncVMTargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  SyncVMMachineFunctionInfo *FuncInfo = MF.getInfo<SyncVMMachineFunctionInfo>();
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i16:
        Register VReg = RegInfo.createVirtualRegister(&SyncVM::GR16RegClass);
...
bool
SyncVMTargetLowering::CanLowerReturn(CallingConv::ID CallConv,
                                     MachineFunction &MF,
                                     bool IsVarArg,
                                     const SmallVectorImpl<ISD::OutputArg> &Outs,
  ...
  return CCInfo.CheckReturn(Outs, RetCC_SyncVM);
}
...
SyncVMTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  if (CallConv == CallingConv::SyncVM_INTR && !Outs.empty())
  ...
  unsigned Opc = (CallConv == CallingConv::SyncVM_INTR ?
                  SyncVMISD::RETI_FLAG : SyncVMISD::RET_FLAG);
...
SDValue SyncVMTargetLowering::LowerCCCCallTo(
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, dl, SyncVM::SP, PtrVT);
...
///
SDValue SyncVMTargetLowering::LowerCallResult(
...
SDValue SyncVMTargetLowering::LowerShifts(SDValue Op,
                                          SelectionDAG &DAG) const {
  ...
  if (Opc == ISD::SRL && ShiftAmount) {
    ...
    // srl A, 1 => clrc; rrc A
    Victim = DAG.getNode(SyncVMISD::RRCL, dl, VT, Victim);
    ShiftAmount -= 1;
  }

  while (ShiftAmount--)
    Victim = DAG.getNode((Opc == ISD::SHL ? SyncVMISD::RLA : SyncVMISD::RRA),
                         dl, VT, Victim);
...
SDValue SyncVMTargetLowering::LowerGlobalAddress(SDValue Op,
  ...
  return DAG.getNode(SyncVMISD::Wrapper, SDLoc(Op), PtrVT, Result);
}
...
SDValue SyncVMTargetLowering::LowerExternalSymbol(SDValue Op,
  ...
  return DAG.getNode(SyncVMISD::Wrapper, dl, PtrVT, Result);
}
...
static SDValue EmitCMP(SDValue &LHS, SDValue &RHS, SDValue &TargetCC,
  ...
  case ISD::SETUGE:
    ...
    if (const ConstantSDNode * C = dyn_cast<ConstantSDNode>(LHS)) {
      ...
      TCC = SyncVMCC::COND_LO;
      break;
    }
    TCC = SyncVMCC::COND_HS;    // aka COND_C
    break;
  case ISD::SETUGT:
    std::swap(LHS, RHS);
    LLVM_FALLTHROUGH;
  case ISD::SETULT:
    ...
    if (const ConstantSDNode * C = dyn_cast<ConstantSDNode>(LHS)) {
      LHS = RHS;
      RHS = DAG.getConstant(C->getSExtValue() + 1, dl, C->getValueType(0));
      TCC = SyncVMCC::COND_HS;
      break;
    }
    TCC = SyncVMCC::COND_LO;    // aka COND_NC
    break;
  case ISD::SETLE:
    std::swap(LHS, RHS);
    LLVM_FALLTHROUGH;
  ...
  return DAG.getNode(SyncVMISD::CMP, dl, MVT::Glue, LHS, RHS);
}
...
SDValue SyncVMTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(SyncVMISD::BR_CC, dl, Op.getValueType(),
                     Chain, Dest, TargetCC, Flag);
}

SDValue SyncVMTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (Convert) {
    SDValue SR = DAG.getCopyFromReg(DAG.getEntryNode(), dl, SyncVM::SR,
                                    MVT::i16, Flag);
    if (Shift)
...
SDValue SyncVMTargetLowering::LowerSIGN_EXTEND(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue
SyncVMTargetLowering::getReturnAddressFrameIndex(SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  SyncVMMachineFunctionInfo *FuncInfo = MF.getInfo<SyncVMMachineFunctionInfo>();
...
SDValue SyncVMTargetLowering::LowerRETURNADDR(SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue SyncVMTargetLowering::LowerFRAMEADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
...
SDValue SyncVMTargetLowering::LowerVASTART(SDValue Op,
                                           SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  SyncVMMachineFunctionInfo *FuncInfo = MF.getInfo<SyncVMMachineFunctionInfo>();
  auto PtrVT = getPointerTy(DAG.getDataLayout());
...
SDValue SyncVMTargetLowering::LowerJumpTable(SDValue Op,
                                             SelectionDAG &DAG) const {
    JumpTableSDNode *JT = cast<JumpTableSDNode>(Op);
    auto PtrVT = getPointerTy(DAG.getDataLayout());
    SDValue Result = DAG.getTargetJumpTable(JT->getIndex(), PtrVT);
    return DAG.getNode(SyncVMISD::Wrapper, SDLoc(JT), PtrVT, Result);
}

/// getPostIndexedAddressParts - returns true by value, base pointer and
/// offset pointer and addressing mode by reference if this node can be
/// combined with a load / store to form a post-indexed load / store.
bool SyncVMTargetLowering::getPostIndexedAddressParts(SDNode *N, SDNode *Op,
                                                      SDValue &Base,
                                                      SDValue &Offset,
...
const char *SyncVMTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((SyncVMISD::NodeType)Opcode) {
  case SyncVMISD::FIRST_NUMBER:       break;
  case SyncVMISD::RET_FLAG:           return "SyncVMISD::RET_FLAG";
  case SyncVMISD::RETI_FLAG:          return "SyncVMISD::RETI_FLAG";
  case SyncVMISD::RRA:                return "SyncVMISD::RRA";
  case SyncVMISD::RLA:                return "SyncVMISD::RLA";
  case SyncVMISD::RRC:                return "SyncVMISD::RRC";
  case SyncVMISD::RRCL:               return "SyncVMISD::RRCL";
  case SyncVMISD::CALL:               return "SyncVMISD::CALL";
  case SyncVMISD::Wrapper:            return "SyncVMISD::Wrapper";
  case SyncVMISD::BR_CC:              return "SyncVMISD::BR_CC";
  case SyncVMISD::CMP:                return "SyncVMISD::CMP";
  case SyncVMISD::SETCC:              return "SyncVMISD::SETCC";
  case SyncVMISD::SELECT_CC:          return "SyncVMISD::SELECT_CC";
  case SyncVMISD::DADD:               return "SyncVMISD::DADD";
  }
  return nullptr;
}

bool SyncVMTargetLowering::isTruncateFree(Type *Ty1,
                                          Type *Ty2) const {
...
bool SyncVMTargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
bool SyncVMTargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
  // SyncVM implicitly zero-extends 8-bit results in 16-bit registers.
  return 0 && Ty1->isIntegerTy(8) && Ty2->isIntegerTy(16);
}

bool SyncVMTargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
  // SyncVM implicitly zero-extends 8-bit results in 16-bit registers.
...
MachineBasicBlock *
SyncVMTargetLowering::EmitShiftInstr(MachineInstr &MI,
  ...
  case SyncVM::Shl8:
    Opc = SyncVM::ADD8rr;
    RC = &SyncVM::GR8RegClass;
    break;
  case SyncVM::Shl16:
    Opc = SyncVM::ADD16rr;
    RC = &SyncVM::GR16RegClass;
    break;
  case SyncVM::Sra8:
    Opc = SyncVM::RRA8r;
    RC = &SyncVM::GR8RegClass;
  ...
  case SyncVM::Rrcl8:
  case SyncVM::Rrcl16: {
    BuildMI(*BB, MI, dl, TII.get(SyncVM::BIC16rc), SyncVM::SR)
      .addReg(SyncVM::SR).addImm(1);
    Register SrcReg = MI.getOperand(1).getReg();
    Register DstReg = MI.getOperand(0).getReg();
    unsigned RrcOpc = MI.getOpcode() == SyncVM::Rrcl16
                    ? SyncVM::RRC16r : SyncVM::RRC8r;
  ...
  Register ShiftAmtReg = RI.createVirtualRegister(&SyncVM::GR8RegClass);
  Register ShiftAmtReg2 = RI.createVirtualRegister(&SyncVM::GR8RegClass);
  ...
  BuildMI(LoopBB, dl, TII.get(SyncVM::PHI), ShiftReg)
    .addReg(SrcReg).addMBB(BB)
    .addReg(ShiftReg2).addMBB(LoopBB);
  BuildMI(LoopBB, dl, TII.get(SyncVM::PHI), ShiftAmtReg)
    .addReg(ShiftAmtSrcReg).addMBB(BB)
    .addReg(ShiftAmtReg2).addMBB(LoopBB);
  if (ClearCarry)
    BuildMI(LoopBB, dl, TII.get(SyncVM::BIC16rc), SyncVM::SR)
      .addReg(SyncVM::SR).addImm(1);
  if (Opc == SyncVM::ADD8rr || Opc == SyncVM::ADD16rr)
    ...
    BuildMI(LoopBB, dl, TII.get(Opc), ShiftReg2)
      .addReg(ShiftReg);
  BuildMI(LoopBB, dl, TII.get(SyncVM::SUB8ri), ShiftAmtReg2)
    .addReg(ShiftAmtReg).addImm(1);
  BuildMI(LoopBB, dl, TII.get(SyncVM::JCC))
    .addMBB(LoopBB)
    .addImm(SyncVMCC::COND_NE);

  // RemBB:
  // DestReg = phi [%SrcReg, BB], [%ShiftReg, LoopBB]
  BuildMI(*RemBB, RemBB->begin(), dl, TII.get(SyncVM::PHI), DstReg)
    .addReg(SrcReg).addMBB(BB)
    .addReg(ShiftReg2).addMBB(LoopBB);
...
SyncVMTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  if (Opc == SyncVM::Shl8  || Opc == SyncVM::Shl16 ||
      Opc == SyncVM::Sra8  || Opc == SyncVM::Sra16 ||
      Opc == SyncVM::Srl8  || Opc == SyncVM::Srl16 ||
      Opc == SyncVM::Rrcl8 || Opc == SyncVM::Rrcl16)
    return EmitShiftInstr(MI, BB);

  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  DebugLoc dl = MI.getDebugLoc();

  assert((Opc == SyncVM::Select16 || Opc == SyncVM::Select8) &&
