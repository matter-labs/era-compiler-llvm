...
uint64_t LLVMSymbolOffsetsEVM(LLVMMemoryBufferRef inBuffer,
  ...
  // five consecutive 32-bit relocations. We only require the first one.
  ...
  for (const ELFSectionRef relocsSec : elfFile->sections()) {
    ...
    // Check relocations with the related sysmbol of the specified type.
    for (const ELFRelocationRef rel : relocsSec.relocations()) {
      ...
      uint32_t symFlags = cantFail(sym->getFlags());
      StringRef symName = cantFail(sym->getName());
      if ((sym->getOther() == llvm::ELF::STO_ERAVM_REFERENCE_SYMBOL) &&
          (symFlags & object::SymbolRef::SF_Undefined) &&
          (symName == subSymName))
