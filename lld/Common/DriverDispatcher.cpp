...
/// Universal linker main(). This linker emulates the gnu, darwin, or
/// windows linker based on the argv[0] or -flavor option.
int unsafeLldMainMemBuf(llvm::ArrayRef<llvm::MemoryBufferRef> inBuffers,
  ...
  // Run the driver. If an error occurs, false will be returned.
  const int r = !d(inBuffers, outBuffer, argsV, stdoutOS, stderrOS, exitEarly,
...
// EraVM local begin
Result lld::lldMainMemBuf(llvm::ArrayRef<llvm::MemoryBufferRef> inBuffers,
                          llvm::raw_pwrite_stream *outBuffer,
                          llvm::ArrayRef<const char *> args,
                          llvm::raw_ostream &stdoutOS,
                          llvm::raw_ostream &stderrOS,
  ...
  int r = 0;
  {
    ...
    llvm::CrashRecoveryContext crc;
    if (!crc.RunSafely([&]() {
          r = unsafeLldMainMemBuf(inBuffers, outBuffer, args, stdoutOS,
                                  stderrOS, drivers, /*exitEarly=*/false);
        }))
      return {crc.RetCode, /*canRunAgain=*/false};
  }
